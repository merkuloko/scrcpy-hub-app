...
export const Header = memo<React.FC<HeaderProps>>(({
  ...
  const connectedCount = useMemo(() => devices.filter((device) => device.state === 'device').length, [devices]);

  return (
    <aside className="mac-sidebar">
      <div className="sidebar-inner">
        <div className="sidebar-brand window-drag-region no-drag">
          ...
          <div className="min-w-0 flex-1">
            <div className="sidebar-brand-title">Scrcpy Hub</div>
            <div className="sidebar-brand-subtitle">{isElectron ? 'macOS utility' : 'web fallback'}</div>
          </div>
        </div>
